...
<head>
    ...
    <title>Bank Garansi Details Form</title>
            ...
            <div class="col-lg-8 col-xl-7">
                ...
                <div class="main-card">
                    ...
                    <div class="header-brand" style="background: linear-gradient(135deg, {{ $isExisting ? '#6366f1 0%, #4338ca' : ($isExtension ? '#10b981 0%, #059669' : '#0d6efd 0%, #0043a8') }} 100%); color: white; padding: 30px;">
                        @if($isExisting)
                        <h3 class="fw-bold mb-1"><i class="bi bi-arrow-repeat me-2"></i>Update Bank Guarantee (Existing)</h3>
                        <p class="mb-0 opacity-75">Update the Amount for the Bank Guarantee: <strong>{{ $existingBg->bg_number ?? '-' }}</strong></p>
                        @elseif($isExtension)
                        <h3 class="fw-bold mb-1"><i class="bi bi-plus-square me-2"></i>New Bank Guarantee Request (Extension)</h3>
                        <p class="mb-0 opacity-75"><i class="bi bi-building me-2"></i>{{ $rec->customer->name }}</p>
                        @else
                        <h3 class="fw-bold mb-1"><i class="bi bi-file-earmark-text me-2"></i>New Bank Guarantee Request</h3>
                        <p class="mb-0 opacity-75"><i class="bi bi-building me-2"></i>{{ $rec->customer->name }}</p>
                        @endif
                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="alert alert-warning d-flex align-items-center shadow-sm" role="alert">
                            ...
                            <div>Please check the fields marked in red.</div>
                        </div>
                        @endif
                        ...
                        <form action="{{ route('customer.portal.store-input', $token) }}" method="POST" id="bgForm">
                            ...
                            <div id="bank-rows">
                                ...
                                <div class="bank-row fade-in" style="background-color: #fff; border: 1px solid #e9ecef; border-radius: 10px; padding: 25px; margin-bottom: 20px; position: relative;">
                                    ...
                                    <div class="btn-remove remove-row" title="Remove Bank" style="position: absolute; top: 15px; right: 15px; color: #dc3545; cursor: pointer; transition: 0.2s; background: #fff0f0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 50%;">
                                    ...
                                    <h6 class="text-primary mb-3 fw-bold">
                                        ...
                                        {{ $isExisting ? 'Bank Guarantee Data' : 'Bank ' . ($index + 1) }}
                                    </h6>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        {{-- CABANG --}}
                                        <div class="col-md-6">
                                            <label class="form-label small fw-bold text-muted">Bank Branch</label>
                                            <input type="text"
                                                name="details[{{ $index }}][branch_name]"
                                                class="form-control"
                                                placeholder="Example: KCU Sudirman"
                                        ...
                                        {{-- ALAMAT --}}
                                        <div class="col-12">
                                            <label class="form-label small fw-bold text-muted">Bank Address</label>
                                        ...
                                        <div class="col-md-6">
                                            <label class="form-label small fw-bold text-muted">Bank Guarantee Amount <span class="text-danger">*</span></label>
                                            <div class="input-group">
                                            ...
                                            @if($isExisting)
                                            <div class="form-text text-primary"><i class="bi bi-info-circle me-1"></i> Please edit this nominal amount as needed.</div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <button type="button" class="btn w-100 py-3 mb-5 btn-outline-primary border-2 fw-bold" id="addBankRow" style="border-style: dashed;">
                                <i class="bi bi-plus-circle-dotted me-2"></i> Add Another Bank
                            </button>
                            @endif
                            ...
                            <div class="sticky-footer rounded-bottom" style="background: white; padding: 20px; border-top: 1px solid #eee; position: sticky; bottom: 0; z-index: 10; box-shadow: 0 -5px 20px rgba(0,0,0,0.05);">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <span class="text-muted fw-bold">Total Bank Guarantee Amount</span>
                                ...
                                <button type="button" id="btnSubmit" class="btn btn-primary w-100 py-3 fw-bold shadow-sm" style="background: {{ $isExisting ? '#4338ca' : ($isExtension ? '#059669' : '#0d6efd') }}; border:none;">
                                    ...
                                    @if($isExisting) Update Nominal
                                    @elseif($isExtension) Request an Extension
                                    @else Save & Proceed
                                    @endif
                                </button>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (btnAdd) {
                btnAdd.addEventListener('click', function() {
                    ...
                    newRow.innerHTML = `
                    <div class="btn-remove remove-row" title="Remove Bank" style="position: absolute; top: 15px; right: 15px; color: #dc3545; cursor: pointer; transition: 0.2s; background: #fff0f0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 50%;"><i class="bi bi-trash"></i></div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted">Bank Name <span class="text-danger">*</span></label>
                            <input type="text" name="details[${tempIndex}][bank_name]" class="form-control" placeholder="Example: BCA / Mandiri" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted">Bank Branch</label>
                            <input type="text" name="details[${tempIndex}][branch_name]" class="form-control" placeholder="Example: KCU Sudirman">
                        </div>
                        <div class="col-12">
                            <label class="form-label small fw-bold text-muted">Bank Address</label>
                            <textarea name="details[${tempIndex}][bank_address]" class="form-control" rows="2" placeholder="Bank address"></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted">Contact Person (Bank)</label>
                            <div class="input-group">
                                <span class="input-group-text bg-white text-muted"><i class="bi bi-telephone"></i></span>
                                <input type="text" name="details[${tempIndex}][contact_person]" class="form-control" placeholder="Bank PIC">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted">Requested Amount <span class="text-danger">*</span></label>
                            <div class="input-group">
            ...
            btnSubmit.addEventListener('click', function(e) {
                ...
                let titleText = isExisting ? 'Update Nominal?' : 'Confirmation Save';
                let msgText = isExisting ?
                    'You will update the nominal for this Bank Guarantee.' :
                    'Are the bank details and nominal you entered correct?';
